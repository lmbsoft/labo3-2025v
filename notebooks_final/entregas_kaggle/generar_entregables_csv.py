...
def process_forecasts_all_csv(directory):
    """
    Procesa TODOS los archivos CSV que tengan las columnas product_id y tn en el directorio dado.
    Hace ensambles y estadísticas varias sobre los forecasts encontrados.
    """
    ...
    with open(summary_filename, 'w', encoding='utf-8') as f:
        f.write(f"Resumen de Ensambles - Todos los CSV válidos\n")
        f.write("="*50 + "\n")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        directory = "entregas_kaggle"
    if not os.path.exists(directory):
        print(f"El directorio '{directory}' no existe.")
        print("Uso: python ensamblar_forecasts.py [directorio]")
        sys.exit(1)
